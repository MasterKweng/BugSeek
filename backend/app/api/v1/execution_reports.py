...
def _validate_environment(db: Session, project_id: int, environment_id: Optional[int]) -> Optional[Environment]:
...
def _validate_version(db: Session, project_id: int, version_id: Optional[int]) -> Optional[Version]:
...
async def get_execution_report_trends(
    ...
    return ApiResponse(data={"group_by": group_by, "items": items})
