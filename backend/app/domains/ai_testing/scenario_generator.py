...
from app.platform.db.base import Project, ApiDefinition
from .schemas import ScenarioDraft, ScenarioSpec, ScenarioNodeSpec
        ...
        try:
            ...
            if not ranked_apis:
                return ScenarioDraft(
                    scenario=ScenarioSpec(name="Empty Scenario", description="No related APIs found"),
                    nodes=[],
...
Path: {api['path']}
Summary: {api['summary']}
TopParams: {api.get('top_level_params', [])}
"""
            ...
            if not selected_api_ids:
                return ScenarioDraft(
                    scenario=ScenarioSpec(name="Empty Scenario", description="AI selected none"),
                    nodes=[],
...
"""
            ...
            if not ai_result.get("success"):
                ...
                try:
                    ...
                    return ScenarioDraft(
                        scenario=ScenarioSpec(name="AI Scenario", description="AI response parse failed"),
                        nodes=[],
                        reasoning=f"AI parse/validate failed: {str(e)}",
                        candidate_apis=selected_apis_with_schema,
                    )

                # normalize with schemas
    ...
    def _extract_json_block(text: str) -> Any:
        # try to find the first JSON object/array block in text
