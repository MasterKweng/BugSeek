...
with engine.begin() as conn:
    ...
    test_data = [
        ...
        (task_id, 1, 'body.user_id', '[{"db_table": "users", "db_column": "id", "score": 0.95, "reasons": ["字段名完全匹配"], "ai_selected": true, "ai_reason": "AI确认"}]'),
        
        # 2. 规则评分的建议 (ai_selected=null)
        (task_id, 1, 'body.order_id', '[{"db_table": "orders", "db_column": "id", "score": 0.88, "reasons": ["字段名匹配"], "ai_selected": null, "ai_reason": null}]'),
        
        # 3. AI选择的建议 (ai_selected=true)
        (task_id, 1, 'body.product_id', '[{"db_table": "products", "db_column": "id", "score": 0.92, "reasons": ["语义匹配"], "ai_selected": true, "ai_reason": "AI推荐"}]'),
        
        # 4. 不同状态的建议
        (task_id, 1, 'query.status', '[{"db_table": "orders", "db_column": "status", "score": 0.75, "reasons": ["字段名匹配"], "ai_selected": null, "ai_reason": null}]'),
        (task_id, 1, 'path.id', '[{"db_table": "users", "db_column": "id", "score": 0.80, "reasons": ["ID字段"], "ai_selected": null, "ai_reason": null}]'),
    ]
    
    for i, (tid, def_id, field_path, candidates) in enumerate(test_data):
        status = 'pending' if i < 3 else ('confirmed' if i == 3 else 'rejected')
        conn.execute(text(f"""
